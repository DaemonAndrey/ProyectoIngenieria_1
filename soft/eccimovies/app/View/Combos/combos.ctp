...
<div>
	<p id="cabeza_tabla_categories">
		...
		<?php foreach ($combos as $combo): ?>
			...
			<?php //if($combo['Product']['subcategory_id']==$subcategory['Subcategory']['id']): ?>
	<div class="row text-center" id="movie-info">
					<?php foreach ($combo['Product'] as $prod): ?>
				<div class="col-md-3">
					<?php echo $this->Html->link($this->Html->image(($prod['code'].'.jpg'), array('alt' =>  $prod['name'], 'class' => 'img-rounded', 'style' => 'width:200px; height:300px', 'id' => 'movies-picture-img')),
                                                    					     array('controller'=>'Products','action' => 'view', $prod['id']),
